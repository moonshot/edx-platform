...
def send_credit_notifications(username, course_key):
    ...
    logo_image = cache.get('attached-logo')
    if not logo_image:
        ...
        if logo_image:
            cache.set('attached-logo', logo_image, settings.CREDIT_NOTIFICATION_CACHE_TIMEOUT)
    else:
    ...
    email_body_content = cache.get('css-email-body')
    if email_body_content is None:
        ...
        if html_file_path:
            with open(html_file_path, 'r') as cur_file:
                ...
                cache.set('css-email-body', email_body_content, settings.CREDIT_NOTIFICATION_CACHE_TIMEOUT)
        else:
            email_body_content = ''
